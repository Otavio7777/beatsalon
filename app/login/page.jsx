...
export default function LoginPage() {
  ...
  const handle = async () => {
    ...
    if (mode === 'signup' && result.data.user) {
      await sb.from('salons').insert({ name: 'Meu Salão', owner_id: result.data.user.id })
    }
  ...
  return (
    <div style={st.page}>
      <div style={st.box}>
        ...
        <input style={st.input} type="password" placeholder="••••••••" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handle()} />
