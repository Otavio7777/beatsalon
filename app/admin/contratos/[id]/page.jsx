...
export default function ContratoDetalhe({ params }) {
  ...
  if (!data?.salon) return <div style={{color:'#fff',padding:40}}>Contrato não encontrado. <Link href="/admin/contratos" style={{color:'#6EE7B7'}}>← Voltar</Link></div>
  ...
  return (
    <div>
      <Link href="/admin/contratos" style={{color:'rgba(255,255,255,.4)',fontSize:13,textDecoration:'none',display:'block',marginBottom:16}}>← Todos os contratos</Link>

      {/* Header do contrato */}
      <div style={{display:'flex',alignItems:'flex-start',gap:16,marginBottom:20,flexWrap:'wrap'}}>
        {salon.logo_url ? (
          <img src={salon.logo_url} alt="" style={{width:60,height:60,borderRadius:14,objectFit:'cover'}}/>
        ) : (
          <div style={{width:60,height:60,borderRadius:14,background:'#1E3A6E',display:'flex',alignItems:'center',justifyContent:'center',fontSize:26,color:'#fff',fontWeight:800,flexShrink:0}}>
            {salon.name?.charAt(0)}
          </div>
        )}
        <div style={{flex:1}}>
          <div style={{fontSize:22,fontWeight:800,color:'#fff',marginBottom:4}}>{salon.name}</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,.35)',marginBottom:10}}>{salon.city||'Sem cidade'} · {PLANOS[salon.plan||'trial']||salon.plan}</div>
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            {/* Toggle ativo */}
            <button onClick={toggleAtivo} disabled={toggling} style={{
              ...
              {toggling ? 'Aguarde...' : ativo ? '🚫 Suspender contrato' : '✅ Reativar contrato'}
            </button>

            {/* Alterar plano */}
            <select defaultValue={salon.plan||'trial'} onChange={e=>savePlan(e.target.value)}
              style={{padding:'8px 14px',borderRadius:10,background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.12)',color:'#fff',fontSize:13,cursor:'pointer'}}>
              {Object.entries(PLANOS).map(([k,v])=><option key={k} value={k}>{v}</option>)}
            </select>
          </div>
        </div>
        <div style={{textAlign:'right'}}>
          <div style={{fontSize:10,color:'rgba(255,255,255,.25)',fontWeight:700,marginBottom:4}}>STATUS</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:14}}>
        {[['Clientes',clients.length,''],['Agendamentos',appts.length,''],['Receita registrada',`R$${receita.toLocaleString('pt-BR')}`,'']].map(([l,v])=>(
          <div key={l} style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,padding:'14px'}}>
            ...
            <div style={{fontSize:20,fontWeight:800,color:l.includes('Receita')?'#6EE7B7':'#fff'}}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={st.card}>
        ...
        {[
          ['ID',salon.id],['Nome',salon.name],['Telefone',salon.phone||'—'],
          ['Endereço',salon.address?`${salon.address}, ${salon.city||''}`:'—'],
          ['Plano',PLANOS[salon.plan||'trial']||salon.plan],
          ['Status',ativo?'Ativo':'Suspenso'],
          ['Cadastro em',salon.created_at?new Date(salon.created_at).toLocaleDateString('pt-BR'):'—'],
        ].map(([k,v])=>(
