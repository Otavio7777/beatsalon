...
import { AlertTriangle, Search, X, Package, Edit, Trash, Plus, ShoppingCart, ArrowDown, ArrowUp } from '../../../lib/icons'
...
function ProdAvatar({ name='?', category }) {
  ...
  return (
    <div style={{width:42,height:42,borderRadius:12,flexShrink:0,background:cat.bg,border:`1.5px solid ${cat.color}22`,
      display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,fontWeight:800,color:cat.color}}>
      {ini}
    </div>
  )
}
...
function StockBadge({ stock, minStock }) {
  const s = stock??0; const m = minStock??5
...
/* Modal de venda / entrada */
function MovimentacaoModal({ produto, tipo, salonId, barberData, onClose, onSaved }) {
  const [qty,   setQty]   = useState(1)
  const [price, setPrice] = useState(tipo==='sale' ? (produto.price||0) : (produto.cost||0))
  const [notes, setNotes] = useState('')
  const [saving, setSaving] = useState(false)
  const [error,  setError]  = useState('')
  const sb = createClient()

  const isVenda = tipo === 'sale'

  const save = async () => {
    if (!qty || qty < 1) { setError('Quantidade deve ser ao menos 1.'); return }
    if (isVenda && (produto.stock||0) < qty) { setError(`Estoque insuficiente. Disponível: ${produto.stock||0}`); return }
    setSaving(true)
    const { error: e } = await sb.rpc('record_product_transaction', {
      p_salon_id:   salonId,
      p_product_id: produto.id,
      p_type:       tipo,
      p_quantity:   Number(qty),
      p_unit_price: Number(price)||0,
      p_notes:      notes || null,
      p_barber_id:  barberData?.id || null,
    })
    if (e) { setError(e.message || 'Erro ao registrar.'); setSaving(false); return }
    onSaved(); onClose()
  }

  const INP = { width:'100%', padding:'11px 14px', borderRadius:10, border:'1.5px solid #E2E8F0', fontSize:14, outline:'none', color:'#0B1E3D', background:'#fff', boxSizing:'border-box', fontFamily:'inherit' }
      ...
      <div className="modal-box">
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}>
          <div style={{width:40,height:40,borderRadius:10,
            background: isVenda ? '#FEF2F2' : '#F0FDF4',
            display:'flex',alignItems:'center',justifyContent:'center'}}>
            {isVenda
              ? <ShoppingCart size={18} color="#DC2626"/>
              : <ArrowDown size={18} color="#059669"/>}
          </div>
          <div>
            <div style={{fontSize:16,fontWeight:800,color:'#0B1E3D'}}>
              {isVenda ? 'Registrar venda' : 'Entrada de estoque'}
            </div>
            <div style={{fontSize:12,color:'#64748B'}}>{produto.name}</div>
          </div>
        </div>

        <div style={{padding:'10px 14px',background:'#F8FAFC',borderRadius:10,marginBottom:4,marginTop:12}}>
          <div style={{fontSize:11,color:'#64748B',marginBottom:2}}>Estoque atual</div>
          <div style={{fontSize:18,fontWeight:800,color:'#0B1E3D'}}>{produto.stock||0} un.</div>
        </div>

        <label style={LBL}>Quantidade *</label>
        <input style={INP} type="number" inputMode="numeric" min="1"
          value={qty} onChange={e=>setQty(Number(e.target.value))} />

        <label style={LBL}>{isVenda ? 'Preço de venda (R$)' : 'Custo unitário (R$)'}</label>
        <input style={INP} type="number" inputMode="decimal"
          value={price} onChange={e=>setPrice(e.target.value)} />

        {qty > 0 && (
          <div style={{padding:'10px 14px',background: isVenda ? '#FEF2F2' : '#F0FDF4',borderRadius:10,marginTop:10,
            fontSize:13,fontWeight:700,color: isVenda ? '#B91C1C' : '#166534'}}>
            Total: R${(Number(qty)*Number(price)).toLocaleString('pt-BR',{minimumFractionDigits:2})}
          </div>
        )}

        <label style={LBL}>Observação</label>
        <input style={INP} placeholder={isVenda ? 'Venda balcão, nome do cliente...' : 'Fornecedor, nota fiscal...'}
          value={notes} onChange={e=>setNotes(e.target.value)} />

        {error && <div style={{padding:'9px 14px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:10,fontSize:13,color:'#B91C1C',marginTop:12}}>{error}</div>}

        <div style={{display:'flex',gap:8,marginTop:20}}>
          <button onClick={save} disabled={saving} style={{
            flex:1, padding:'13px', borderRadius:11, border:'none',
            background: saving ? '#E2E8F0' : isVenda ? '#DC2626' : '#059669',
            color: saving ? '#94A3B8' : '#fff', fontSize:14, fontWeight:700, cursor: saving ? 'default' : 'pointer',
          }}>
            {saving ? 'Registrando...' : isVenda ? 'Confirmar venda' : 'Confirmar entrada'}
          </button>
          <button onClick={onClose} style={{padding:'13px 18px',borderRadius:11,border:'1.5px solid #E2E8F0',background:'#fff',color:'#64748B',fontSize:13,cursor:'pointer'}}>
            Cancelar
          </button>
        </div>
      </div>
...
function ProdutoModal({ salonId, produto, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      stock:Number(form.stock)||0, min_stock:Number(form.min_stock)||5,
    }
  ...
  const INP = { width:'100%', padding:'11px 14px', borderRadius:10, border:'1.5px solid #E2E8F0', fontSize:14, outline:'none', color:'#0B1E3D', background:'#fff', boxSizing:'border-box', fontFamily:'inherit' }
  const LBL = { display:'block', fontSize:10, fontWeight:700, color:'#64748B', textTransform:'uppercase', letterSpacing:'.5px', marginBottom:4, marginTop:14 }
    ...
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          <div><label style={LBL}>Preço de venda (R$)</label><input style={INP} type="number" inputMode="decimal" value={form.price} onChange={e=>set('price',e.target.value)} placeholder="0,00"/></div>
          <div><label style={LBL}>Custo (R$)</label><input style={INP} type="number" inputMode="decimal" value={form.cost} onChange={e=>set('cost',e.target.value)} placeholder="0,00"/></div>
          <div><label style={LBL}>Estoque atual</label><input style={INP} type="number" inputMode="numeric" value={form.stock} onChange={e=>set('stock',e.target.value)} placeholder="0"/></div>
          <div><label style={LBL}>Estoque mínimo</label><input style={INP} type="number" inputMode="numeric" value={form.min_stock} onChange={e=>set('min_stock',e.target.value)} placeholder="5"/></div>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:16,padding:'12px 14px',background:'#F8FAFC',borderRadius:10}}>
          ...
          <div onClick={()=>set('active',!form.active)} style={{width:44,height:24,borderRadius:12,cursor:'pointer',position:'relative',background:form.active?'#059669':'#CBD5E1',transition:'background .2s'}}>
        ...
        <div style={{display:'flex',gap:8,marginTop:20}}>
          <button onClick={save} disabled={saving||!form.name.trim()} style={{flex:1,padding:'13px',borderRadius:11,border:'none',background:saving||!form.name.trim()?'#E2E8F0':'#0B1E3D',color:saving||!form.name.trim()?'#94A3B8':'#fff',fontSize:14,fontWeight:700,cursor:'pointer'}}>
            {saving ? 'Salvando...' : produto?.id ? 'Salvar alterações' : 'Adicionar produto'}
          </button>
          <button onClick={onClose} style={{padding:'13px 18px',borderRadius:11,border:'1.5px solid #E2E8F0',background:'#fff',color:'#64748B',fontSize:13,cursor:'pointer'}}>Cancelar</button>
        </div>
      </div>
    </div>
  )
...
function ProdCard({ p, margem, onEdit, onDel, onVenda, onEntrada, isBarber }) {
  const cat = catInfo(p.category)
  return (
    <div style={{padding:'13px 16px',borderBottom:'1px solid #F1F5F9'}}>
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom: isBarber ? 10 : 0}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
            ...
            {!isBarber&&p.price>0&&<span style={{fontSize:11,color:margem(p)>30?'#059669':'#D97706',fontWeight:600}}>{margem(p)}% mg</span>}
            <StockBadge stock={p.stock} minStock={p.min_stock}/>
          </div>
        </div>
        {!isBarber&&(
          <div style={{display:'flex',gap:6,flexShrink:0}}>
            <button onClick={onEdit} style={{width:38,height:38,borderRadius:10,border:'1.5px solid #E2E8F0',background:'#F8FAFC',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <Edit size={14} color="#64748B"/>
            </button>
            <button onClick={onDel} style={{width:38,height:38,borderRadius:10,border:'1.5px solid #FCA5A5',background:'#FEF2F2',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      {p.active && (
        <div style={{display:'flex',gap:6,marginTop:isBarber?0:10}}>
          <button onClick={onVenda} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,
            padding:'8px',borderRadius:9,border:'1.5px solid #FCA5A5',background:'#FEF2F2',
            color:'#DC2626',fontSize:12,fontWeight:700,cursor:'pointer'}}>
            <ShoppingCart size={13}/> Venda
          </button>
          <button onClick={onEntrada} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,
            padding:'8px',borderRadius:9,border:'1.5px solid #BBF7D0',background:'#F0FDF4',
...
export default function ProdutosPage() {
  const { salon, user, loading: salonLoading, isBarber, barberData } = useSalon()
  ...
  const [movModal, setMovModal]     = useState(null) // { produto, tipo }
  const sb = createClient()
  ...
  return (
    <div className="pg">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,gap:10}}>
        <div>
          ...
          <div className="pg-sub">
            {isBarber ? 'Estoque e vendas' : `Estoque e catálogo · ${salon?.name}`}
          </div>
        </div>
        {!isBarber && (
      ...
      {/* KPIs */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10,marginBottom:16}} className="produtos-kpis">
        {[
      ...
      {baixoEstoque.length>0&&(
        <div style={{display:'flex',gap:10,alignItems:'flex-start',background:'#FFFBEB',border:'1px solid #FDE68A',borderRadius:12,padding:'12px 14px',marginBottom:16}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:'#B45309'}}>{baixoEstoque.map(p=>p.name).join(' · ')}</div>
          </div>
        </div>
      )}

      {/* Busca */}
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'0 12px',borderRadius:10,border:'1.5px solid #E2E8F0',background:'#fff',minHeight:44,marginBottom:10}}>
      ...
      <div style={{overflowX:'auto',paddingBottom:4,marginBottom:14,WebkitOverflowScrolling:'touch'}}>
        <div style={{display:'flex',gap:6,minWidth:'max-content',paddingRight:4}}>
          <button onClick={()=>setCatFilter('todos')} style={{padding:'7px 14px',borderRadius:20,fontSize:11,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',
            border:`1.5px solid ${catFilter==='todos'?'#0B1E3D':'#E2E8F0'}`,background:catFilter==='todos'?'#0B1E3D':'#fff',color:catFilter==='todos'?'#fff':'#64748B'}}>
            Todos ({produtos.length})
          </button>
          {CATS.map(cat=>(
            <button key={cat.id} onClick={()=>setCatFilter(catFilter===cat.id?'todos':cat.id)} style={{padding:'7px 14px',borderRadius:20,fontSize:11,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',
              border:`1.5px solid ${catFilter===cat.id?cat.color:'#E2E8F0'}`,background:catFilter===cat.id?cat.bg:'#fff',color:catFilter===cat.id?cat.color:'#64748B'}}>
      ...
      {/* Lista */}
      {loading ? (
        <div style={{background:'#fff',borderRadius:14,border:'1px solid #E2E8F0',padding:'40px 16px',textAlign:'center',color:'#94A3B8',fontSize:13}}>Carregando produtos...</div>
      ) : filtered.length===0 ? (
        <div style={{background:'#fff',borderRadius:14,border:'1px solid #E2E8F0',padding:'48px 20px',textAlign:'center'}}>
          <div style={{display:'flex',justifyContent:'center',marginBottom:12,opacity:.25}}><Package size={40}/></div>
          ...
          {!search&&catFilter==='todos'&&!isBarber&&(
        ...
        <>
          ...
          <div className="produtos-list-mobile" style={{background:'#fff',borderRadius:14,border:'1px solid #E2E8F0',overflow:'hidden'}}>
            {filtered.map(p=>(
              <ProdCard key={p.id} p={p} margem={margem} isBarber={isBarber}
                onEdit={()=>openEdit(p)} onDel={()=>del(p.id)}
                onVenda={()=>setMovModal({produto:p,tipo:'sale'})}
                onEntrada={()=>setMovModal({produto:p,tipo:'entry'})}/>
            ))}
          </div>
          ...
          <div className="produtos-table-desktop tbl-wrap">
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead style={{background:'#F8FAFC',borderBottom:'1px solid #E2E8F0'}}>
                <tr>
                  {['Produto','Categoria','Preço',...(!isBarber?['Custo','Margem']:[]),'Estoque','Ações'].map(h=>(
                ...
                {filtered.map((p,i)=>{
                  ...
                  return (
                    <tr key={p.id} style={{borderBottom:i<filtered.length-1?'1px solid #F1F5F9':'none'}}>
                      ...
                      <td style={{padding:'13px 14px'}}><StockBadge stock={p.stock} minStock={p.min_stock}/></td>
                      <td style={{padding:'13px 14px'}}>
                        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                          {p.active&&(
                            <>
                              <button onClick={()=>setMovModal({produto:p,tipo:'sale'})} style={{
                                display:'flex',alignItems:'center',gap:5,padding:'6px 10px',borderRadius:8,
                                border:'1.5px solid #FCA5A5',background:'#FEF2F2',color:'#DC2626',fontSize:11,fontWeight:700,cursor:'pointer'}}>
                                <ShoppingCart size={12}/> Venda
                              </button>
                              <button onClick={()=>setMovModal({produto:p,tipo:'entry'})} style={{
                                display:'flex',alignItems:'center',gap:5,padding:'6px 10px',borderRadius:8,
                                border:'1.5px solid #BBF7D0',background:'#F0FDF4',color:'#059669',fontSize:11,fontWeight:700,cursor:'pointer'}}>
                                <ArrowDown size={12}/> Entrada
                              </button>
                            </>
                          )}
                          {!isBarber&&<>
                            <button onClick={()=>openEdit(p)} style={{padding:'6px 12px',borderRadius:8,border:'1px solid #E2E8F0',background:'#fff',fontSize:12,fontWeight:600,cursor:'pointer',color:'#64748B',display:'flex',alignItems:'center',gap:5}}>
                              <Edit size={12} color="#64748B"/> Editar
                            </button>
                            <button onClick={()=>del(p.id)} style={{width:32,height:32,borderRadius:8,border:'1px solid #FCA5A5',background:'#FEF2F2',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      {showModal&&salon&&(
        <ProdutoModal salonId={salon.id} produto={editProd} onClose={()=>setShowModal(false)} onSaved={load}/>
      )}
      {movModal&&salon&&(
        <MovimentacaoModal
