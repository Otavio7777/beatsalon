...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (!salon?.id) return
    createClient().rpc('auto_complete_past_appointments').catch(()=>null)
  }, [salon?.id])
    ...
    <div style={{display:'flex',minHeight:'100vh',position:'relative'}}>
      ...
      <aside className={`sidebar ${open?'open':'closed'}`}>
        <div className="sidebar-brand" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between'}}>
          ...
          {mobile&&<button onClick={()=>setOpen(false)} style={{background:'rgba(255,255,255,.08)',border:'none',borderRadius:8,width:30,height:30,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'rgba(255,255,255,.6)',marginTop:2}}><X size={15} color="currentColor"/></button>}
        </div>

        {/* Nome do salão / modo manutenção */}
        <div style={{padding:'6px 16px 4px',display:'flex',alignItems:'center',gap:6}}>
          {maintenanceMode && <span style={{width:6,height:6,borderRadius:3,background:'#F59E0B',display:'inline-block',flexShrink:0}}/>}
        ...
        <nav style={{padding:'6px 0',flex:1,overflowY:'auto'}}>
          ...
          {isAdmin&&(
            <>
              <div className="nav-section-label" style={{marginTop:8}}>Administrador</div>
              {maintenanceMode ? (
                <button onClick={exitMaintenance} className="nav-item"
                  ...
                  <ArrowLeft size={15} color="#F59E0B"/> Sair da manutenção
                </button>
              ) : (
                <Link href="/admin" className="nav-item" onClick={()=>mobile&&setOpen(false)}
                  style={{color:'rgba(245,158,11,.8)'}}>
                  <Settings size={15} color="rgba(245,158,11,.8)"/> Painel Admin
                </Link>
              )}
            </>
          )}
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>
        ...
        {maintenanceMode&&(
          <div style={{background:'#78350F',borderBottom:'2px solid #D97706',padding:'8px 16px',display:'flex',alignItems:'center',gap:12}}>
            ...
            <div style={{flex:1,fontSize:13,color:'#FDE68A',fontWeight:600}}>
              Modo manutenção — <strong style={{color:'#fff'}}>{maintenanceName}</strong>
              <span style={{fontSize:11,color:'rgba(253,230,138,.5)',fontWeight:400,marginLeft:8}}>Alterações afetam este salão.</span>
            </div>
            <button onClick={exitMaintenance}
              style={{padding:'5px 14px',background:'rgba(245,158,11,.2)',border:'1px solid rgba(245,158,11,.35)',borderRadius:8,color:'#FDE68A',fontSize:12,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}>
              <ArrowLeft size={12} color="#FDE68A"/> Voltar ao Admin
            </button>
          </div>
        )}
        ...
        <header className="top-header">
          ...
          <div className="header-title">
            <cur.Icon size={15} color="var(--navy-600)" style={{marginRight:6}}/>{cur.label}
          </div>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            {isAdmin&&!maintenanceMode&&(
              <Link href="/admin" style={{fontSize:11,fontWeight:700,padding:'4px 10px',borderRadius:20,background:'rgba(245,158,11,.1)',color:'#D97706',border:'1px solid rgba(245,158,11,.2)',textDecoration:'none'}}>
                Admin
              </Link>
            )}
